...
$string = "PublicIpName,SYN-PPSThreshold,TCP-PPSThreshold,UDP-PPSThreshold,PublicIpResourceId"
...
foreach($PIP in $PIPs){
    ...
    if($tcptrigger -ne $null){
        ...
        $string = "$($PIP.Name),$($syntrigger[59].Maximum),$($tcptrigger[59].Maximum),$($udptrigger[59].Maximum),$($PIP.Id)"
        $string | Out-File $outputFile -append -force
    }
